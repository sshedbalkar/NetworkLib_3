    ...
    void OnClientDisconnect(
        ...
        if (client) {
            if (m_mapPlayerRoster.find(client->GetID()) ==
                ...
                merci::logging::INFO()
                    << "[UNGRACEFUL REMOVAL]:" + std::to_string(pd.nUniqueID)
                    << std::endl;
    ...
    void OnMessage(std::shared_ptr<olc::net::connection<GameMsg>> client,
        ...
        if (!m_vGarbageIDs.empty()) {
            for (auto pid : m_vGarbageIDs) {
                ...
                m << pid;
                merci::logging::INFO() << "Removing " << pid << std::endl;
                MessageAllClients(m);
            }
            m_vGarbageIDs.clear();
        }
        ...
        switch (msg.header.id) {
            ...
            case GameMsg::Dummy_Message: {
                ...
                msg >> clientmsg;
                merci::logging::INFO()
                    << "[Server][Dummy_Message]: " << clientmsg << std::endl;
                // Send reply
                olc::net::message<GameMsg> msgReply;
                msgReply.header.id = GameMsg::Dummy_Message;
                msgReply << "You said: " << clientmsg;
